import { IconNext } from "@V/designSystem";
import { mount } from "@vue/test-utils";

describe("IconNext", () => {
	it("renders the next svg icon with default props", () => {
		const wrapper = mount(IconNext);
		...
		expect(svg.attributes("data-dfv-icon")).toBe("next");
		expect(svg.attributes("style")).toContain("--DFV-icon-size: 1rem");

		expect(path.attributes("d")).toBe("M7 5L13 10L7 15");
